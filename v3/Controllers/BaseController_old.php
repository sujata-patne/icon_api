...
require_once(APP."Utils/KLogger.php");
...
class BaseController {
    ...
    public function __construct( $body = '', $status = 200, $headers = array() ) {
    		...
    		$this->write($body);
			$this->successLog = new KLogger (SUCCESSLOGS. date('d-m-Y').'.log' , KLogger::INFO ); 
			$this->errorLog = new KLogger (ERRORLOGS .date('d-m-Y').'.log' , KLogger::INFO );
			
    }
 public function logCurlAPI($extractParams){

 $logFile = LOGS.'CurlControllers_'.date('d-m-Y-H').'.log';
